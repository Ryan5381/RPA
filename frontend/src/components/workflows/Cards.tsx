      ...
      {items.map((item) => {
        ...
        return (
          <div
            ...
            className={`relative border rounded-xl p-5 cursor-pointer transition-all duration-300 flex items-center gap-4 group overflow-hidden select-none ${
              isSelected
          ...
          >
            {/* Hover 發光背景效果 */}
            <div className="absolute inset-0 bg-linear-to-r from-cyan-500/5 to-blue-500/5 opacity-0 transition-opacity duration-300 group-hover:opacity-100 pointer-events-none" />

            {/* 圖示外框 */}
            <div
              className={`w-11 h-11 rounded-lg flex items-center justify-center transition-colors duration-300 shrink-0 ${
                isSelected
                  ? "bg-cyan-950/80 text-cyan-400 border border-cyan-500/30"
                  : "bg-slate-950/50 text-slate-400 group-hover:text-slate-300"
              }`}
            >
            ...
            <div className="flex-1 min-w-0">
              <p
                className={`text-sm font-bold tracking-wide transition-colors duration-300 truncate ${
                  isSelected
                    ? "text-cyan-400"
